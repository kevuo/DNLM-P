...
Mat NLMFilterDeceived::nlmfilterDeceived(const Mat& A, const Mat& L, int w, int w_n, double sigma_s, int sigma_r, int num_threads){
    double minA, maxA;
    ...
    if (type == CV_32FC3 )
       //B = bfltColorDeceived(A, L, w, sigma(1),sigma(2));//?????????????????
        B = this->nlmfltBWDeceived(A, L, w, w_n, sigma_s, sigma_r, num_threads);
    else
        ;
    return B;
}
...
Mat NLMFilterDeceived::nlmfltBWDeceived(const Mat& A, const Mat& L, int w, int w_n, double sigma_d, int sigma_r, int num_threads){
    Mat B,C,D;
    ...
    C = this->nlmfilBW_deceived(B, L, w, w_n, sigma_d,sigma_r, num_threads);
...
Mat NLMFilterDeceived::nlmfilBW_deceived(const Mat& A, const Mat& Laplacian, int w, int w_n, double sigma_d, int sigma_r, int num_threads){
